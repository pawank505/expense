...
const EditGroup = ({ group, editform, setEditForm, user }) => {
  ...
  const [groupData, setGroupData] = useState({
    name: '',
    description: '',
    members: [], // This will only contain email strings
    groupId: group._id,
    createdBy: userId
  });
  ...
  // Helper function to extract email from member object or return the string
  ...
  useEffect(() => {
    if (group) {
      // Ensure members array contains only email strings
      ...
      setGroupData({
        ...
        members: memberEmails, // Only email strings
  ...
  // Remove email from members array
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/group/${group._id}`,
        {
          ...
          // Ensure members array contains only unique, valid emails
